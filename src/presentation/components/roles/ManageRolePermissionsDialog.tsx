...
import {
  useBatchGrantRolePermission,
  useBatchRevokeRolePermission,
  useRolePermissions,
...
export default function ManageRolePermissionsDialog({
  roleName,
  open,
  onClose,
  ...
  const batchGrant = useBatchGrantRolePermission();
  const batchRevoke = useBatchRevokeRolePermission();
  ...
  const handleToggle = (permissionName: string) => {
    ...
    // If pending is defined, it represents the intended state; otherwise assume current
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (grantList.length) {
        requests.push(
          batchGrant.mutateAsync({ roleName, permissionNames: grantList }),
        );
      }
      if (revokeList.length) {
        requests.push(
          batchRevoke.mutateAsync({ roleName, permissionNames: revokeList }),
        );
      }
